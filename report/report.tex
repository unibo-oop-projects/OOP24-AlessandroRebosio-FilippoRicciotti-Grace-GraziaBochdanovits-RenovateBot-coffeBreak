...
%
All’inizio di ogni partita, Mario dispone di tre vite. 
Ogni volta che entra in contatto con un nemico, come barili rotolanti o fiamme, perde una vita e viene riposizionato 
all’inizio del livello corrente, mantenendo però il punteggio ottenuto fino a quel momento. 
%
...
All'avvio dell'applicazione, l'unico strumento a disposizione sarà la tastiera. Ci si troverà nel menu principale, dove sarà possibile visualizzare la leaderboard. Le uniche azioni disponibili saranno:

\begin{itemize}
	\item Avviare una nuova partita tramite il tasto \texttt{ENTER}
	\item Uscire dal gioco premendo il tasto \texttt{Q}
\end{itemize}

\section{Durante il Gioco}
Dopo aver premuto \texttt{ENTER}, inizierà il primo livello. L'obiettivo del giocatore è raggiungere la principessa, raccogliendo power-up ed evitando i nemici.

\subsection*{Comandi disponibili:}
\begin{itemize}
	\item Movimento del personaggio:
	      \begin{itemize}
		      \item \texttt{FRECCIA SU}: Muovi in alto (se solo se in collisione con una scale)
		      \item \texttt{FRECCIA GIÙ}: Muovi in basso (se solo se sotto il personaggio c'è una scala)
		      \item \texttt{FRECCIA DESTRA}: Muovi a destra
		      \item \texttt{FRECCIA SINISTRA}: Muovi a sinistra
	      \end{itemize}
	\item \texttt{BARRA SPAZIATRICE}: Salto
	\item \texttt{ESC}: Pausa del gioco
\end{itemize}

\section{Menu di Pausa}
Nel menu di pausa saranno disponibili le seguenti opzioni:
\begin{itemize}
	\item \texttt{ENTER}: Riprendi il livello
	\item \texttt{ESC}: Esci dal livello (torna al menu principale)
	\item \texttt{Q}: Esci dal gioco
\end{itemize}

\section{Game Over}
Quando il personaggio perderà tutte le vite, si entrerà nella schermata di Game Over dove:
\begin{itemize}
	\item Dovrai inserire il tuo nome (3 lettere) e premere \texttt{ENTER}
	\item Il nome verrà richiesto solo se il punteggio è tra i primi 5
	\item Dopo l'inserimento si tornerà al menu principale
	\item Il punteggio verrà salvato nella leaderboard
\end{itemize}
